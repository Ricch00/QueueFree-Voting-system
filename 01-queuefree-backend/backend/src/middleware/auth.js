...
const authStudent = async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.query('SELECT id, student_id, full_name, email, verification_status, is_active, device_fingerprint FROM students WHERE id = ?', [decoded.id]);
    if (!rows.length || !rows[0].is_active) return res.status(401).json({ success: false, message: 'Account not found or disabled' });
    req.student = rows[0];
    next();
  } catch (err) {
...
const authAdmin = async (req, res, next) => {
  try {
    ...
    if (!rows.length || !rows[0].is_active) return res.status(401).json({ success: false, message: 'Admin not found or disabled' });
    req.admin = rows[0];
    next();
  } catch (err) {
    return res.status(401).json({ success: false, message: err.name === 'TokenExpiredError' ? 'Token expired' : 'Invalid token' });
