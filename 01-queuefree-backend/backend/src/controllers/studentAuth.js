...
const register = async (req, res) => {
  ...
  try {
    ...
    const [existing] = await conn.query('SELECT id FROM students WHERE email = ? OR student_id = ?', [email, student_id]);
    if (existing.length) {
      await conn.rollback();
    ...
    if (device_fingerprint) {
      const [devCheck] = await conn.query('SELECT id FROM device_registry WHERE device_fingerprint = ?', [device_fingerprint]);
      if (devCheck.length) {
        await conn.rollback();
    ...
    if (device_fingerprint) {
      await conn.query('INSERT IGNORE INTO device_registry (device_fingerprint, first_student_id) VALUES (?, ?)', [device_fingerprint, result.insertId]);
    }

    if (id_photo || selfie) {
      const idPhotoUrl = id_photo ? await saveBase64Image(id_photo, `${result.insertId}-id-${Date.now()}.${getImageExtension(id_photo)}`) : null;
      const selfieUrl = selfie ? await saveBase64Image(selfie, `${result.insertId}-selfie-${Date.now()}.${getImageExtension(selfie)}`) : null;
      await conn.query(
        'UPDATE students SET id_photo_url = COALESCE(?, id_photo_url), selfie_url = COALESCE(?, selfie_url), verification_status = ? WHERE id = ?',
        [idPhotoUrl, selfieUrl, 'pending', result.insertId]
      );
    }

    await conn.commit();
    await auditLog({ actorType: 'student', actorId: result.insertId, actorEmail: email, action: 'REGISTER', req });
...
const login = async (req, res) => {
  try {
    ...
    if (!valid) {
      ...
      await pool.query('UPDATE students SET login_attempts = ?, locked_until = ? WHERE id = ?', [attempts, lockUntil, student.id]);
    ...
    if (device_fingerprint) {
      ...
      const [devReg] = await pool.query('SELECT first_student_id FROM device_registry WHERE device_fingerprint = ?', [device_fingerprint]);
      ...
      if (!student.device_fingerprint) {
        await pool.query('UPDATE students SET device_fingerprint = ? WHERE id = ?', [device_fingerprint, student.id]);
        await pool.query('INSERT IGNORE INTO device_registry (device_fingerprint, first_student_id) VALUES (?, ?)', [device_fingerprint, student.id]);
      }
    }

    await pool.query('UPDATE students SET login_attempts = 0, locked_until = NULL WHERE id = ?', [student.id]);
...
const getProfile = async (req, res) => {
  try {
    const [rows] = await pool.query(
      'SELECT id, student_id, full_name, email, phone, program, level, department, faculty, profile_photo, id_photo_url, selfie_url, verification_status, device_fingerprint, created_at FROM students WHERE id = ?',
      [req.student.id]
    );
...
const updateProfile = async (req, res) => {
  try {
    ...
    await pool.query('UPDATE students SET full_name = ?, phone = ? WHERE id = ?', [full_name, phone || null, req.student.id]);
...
const changePassword = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT password FROM students WHERE id = ?', [req.student.id]);
    ...
    await pool.query('UPDATE students SET password = ? WHERE id = ?', [await bcrypt.hash(new_password, 12), req.student.id]);
...
const uploadDocuments = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT student_id FROM students WHERE id = ?', [req.student.id]);
    ...
    const updates = [];
    const params = [];
    if (idPhotoUrl) { updates.push('id_photo_url = ?'); params.push(idPhotoUrl); }
    if (selfieUrl) { updates.push('selfie_url = ?'); params.push(selfieUrl); }
    updates.push('verification_status = ?'); params.push('pending');
