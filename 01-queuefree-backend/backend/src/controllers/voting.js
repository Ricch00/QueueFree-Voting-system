...
const getVotingToken = async (req, res) => {
  try {
    ...
    if (!elections.rows.length) return res.status(404).json({ success: false, message: 'Election not active' });
    if (new Date() > new Date(elections.rows[0].end_date)) return res.status(400).json({ success: false, message: 'Election has ended' });

    const voted = await pool.query('SELECT id FROM voter_records WHERE student_id = $1 AND election_id = $2', [student.id, election_id]);
    if (voted.rows.length) return res.status(400).json({ success: false, message: 'You have already voted', already_voted: true });
    ...
    if (tokens.rows.length) return res.json({ success: true, data: { token: tokens.rows[0].token, election_id: parseInt(election_id), expires_at: tokens.rows[0].expires_at } });
    ...
    await pool.query('INSERT INTO voting_tokens (token, student_id, election_id, device_fingerprint, expires_at) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (student_id, election_id) DO NOTHING',
      [newToken, student.id, election_id, student.device_fingerprint, elections.rows[0].end_date]);
...
const castVote = async (req, res) => {
  ...
  try {
    ...
    if (!tokens.rows.length) { await conn.rollback(); return res.status(400).json({ success: false, message: 'Invalid voting token' }); }
    ...
    if (!elections.rows.length) { await conn.rollback(); return res.status(400).json({ success: false, message: 'Election not active' }); }
    ...
    if (alreadyVoted.rows.length) { await conn.rollback(); return res.status(400).json({ success: false, message: 'Already voted' }); }
    ...
    for (const vote of votes) {
      ...
      if (!check.rows.length) { await conn.rollback(); return res.status(400).json({ success: false, message: `Invalid candidate ${vote.candidate_id}` }); }
    }
    ...
    if (student.device_fingerprint) {
      ...
      if (devReg.rows.length) {
        let voted = [];
        try { voted = JSON.parse(devReg.rows[0].elections_voted || '[]'); } catch (_) {}
...
const getResults = async (req, res) => {
  try {
    ...
    const election = elections.rows[0];
    ...
    if (election.status !== 'results_published') {
      ...
      if (!voted.rows.length) return res.status(403).json({ success: false, message: 'You must vote before viewing results', must_vote_first: true });
    }
...
const buildResults = async (election_id) => {
  ...
  for (const pos of positions.rows) {
    ...
    const total = candidates.rows.reduce((s, c) => s + parseInt(c.vote_count), 0);
    results.push({
      position: pos,
      candidates: candidates.rows.map(c => ({ ...c, percentage: total > 0 ? ((c.vote_count / total) * 100).toFixed(1) : '0.0' })),
      total_votes: total
    });
  }
  return results;
...
const checkVotingStatus = async (req, res) => {
  try {
    ...
    res.json({ success: true, data: { has_voted: voted.rows.length > 0, voted_at: voted.rows[0]?.voted_at || null, has_token: token.rows.length > 0, token_used: token.rows[0]?.is_used || false } });
  } catch (err) { res.status(500).json({ success: false, message: 'Failed' }); }
};
