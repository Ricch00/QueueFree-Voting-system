...
const getVotingToken = async (req, res) => {
  try {
    ...
    const [elections] = await pool.query("SELECT * FROM elections WHERE id = ? AND status = 'active'", [election_id]);
    ...
    const [voted] = await pool.query('SELECT id FROM voter_records WHERE student_id = ? AND election_id = ?', [student.id, election_id]);
    if (voted.length) return res.status(400).json({ success: false, message: 'You have already voted', already_voted: true });

    const [tokens] = await pool.query('SELECT * FROM voting_tokens WHERE student_id = ? AND election_id = ? AND is_used = 0 AND expires_at > NOW()', [student.id, election_id]);
...
const castVote = async (req, res) => {
  ...
  try {
    ...
    // Lock the token row
    const [tokens] = await conn.query('SELECT * FROM voting_tokens WHERE token = ? AND student_id = ? AND election_id = ? FOR UPDATE', [voting_token, student.id, election_id]);
    ...
    const [elections] = await conn.query("SELECT * FROM elections WHERE id = ? AND status = 'active'", [election_id]);
    if (!elections.length) { await conn.rollback(); return res.status(400).json({ success: false, message: 'Election not active' }); }

    // Check not already voted
    const [alreadyVoted] = await conn.query('SELECT id FROM voter_records WHERE student_id = ? AND election_id = ?', [student.id, election_id]);
    if (alreadyVoted.length) { await conn.rollback(); return res.status(400).json({ success: false, message: 'Already voted' }); }

    // Validate candidates
    for (const vote of votes) {
      const [check] = await conn.query("SELECT id FROM candidates WHERE id = ? AND position_id = ? AND election_id = ? AND status = 'approved'",
    ...
    // Mark token used
    await conn.query('UPDATE voting_tokens SET is_used = 1, used_at = NOW() WHERE id = ?', [tokenRow.id]);

    // Record voter participation (anonymised - no link to what they voted)
    await conn.query('INSERT INTO voter_records (student_id, election_id, device_fingerprint, ip_address) VALUES (?, ?, ?, ?)',
    ...
    for (const vote of votes) {
      await conn.query('INSERT INTO votes (election_id, position_id, candidate_id, token_id, vote_hash) VALUES (?, ?, ?, ?, ?)',
        [election_id, vote.position_id, vote.candidate_id, tokenRow.id, voteHash]);
    }

    // Update device registry
    if (student.device_fingerprint) {
      const [devReg] = await conn.query('SELECT elections_voted FROM device_registry WHERE device_fingerprint = ?', [student.device_fingerprint]);
      if (devReg.length) {
        let voted = [];
        ...
        if (!voted.includes(parseInt(election_id))) {
          ...
          await conn.query('UPDATE device_registry SET elections_voted = ? WHERE device_fingerprint = ?', [JSON.stringify(voted), student.device_fingerprint]);
        }
      }
    }
...
const getResults = async (req, res) => {
  try {
    ...
    if (election.status !== 'results_published') {
      const [voted] = await pool.query('SELECT id FROM voter_records WHERE student_id = ? AND election_id = ?', [req.student.id, election_id]);
    ...
    const [[stats]] = await pool.query('SELECT COUNT(*) as total_votes FROM voter_records WHERE election_id = ?', [election_id]);
    const [[eligible]] = await pool.query("SELECT COUNT(*) as total_eligible FROM students WHERE verification_status='verified' AND is_active=1");
    const turnout = eligible.total_eligible > 0 ? ((stats.total_votes / eligible.total_eligible) * 100).toFixed(1) : 0;

    res.json({ success: true, data: { election, results, stats: { total_votes: stats.total_votes, total_eligible: eligible.total_eligible, turnout } } });
...
const adminGetResults = async (req, res) => {
  try {
    ...
    const [[tokens]] = await pool.query('SELECT COUNT(*) as count FROM voting_tokens WHERE election_id = ? AND is_used=1', [election_id]);
    res.json({ success: true, data: { results, stats: { votes_cast: votes.count, tokens_used: tokens.count } } });
...
const checkVotingStatus = async (req, res) => {
  try {
    const { election_id } = req.params;
    const [voted] = await pool.query('SELECT voted_at FROM voter_records WHERE student_id = ? AND election_id = ?', [req.student.id, election_id]);
    const [token] = await pool.query('SELECT token, is_used, expires_at FROM voting_tokens WHERE student_id = ? AND election_id = ?', [req.student.id, election_id]);
