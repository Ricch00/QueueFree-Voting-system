...
const getAllStudents = async (req, res) => {
  try {
    ...
    let paramIndex = 1;
    if (search) { where += ` AND (s.full_name LIKE $${paramIndex++} OR s.student_id LIKE $${paramIndex++} OR s.email LIKE $${paramIndex++})`; params.push(`%${search}%`, `%${search}%`, `%${search}%`); }
    if (status) { where += ` AND s.verification_status = $${paramIndex++}`; params.push(status); }
    if (level) { where += ` AND s.level = $${paramIndex++}`; params.push(level); }
...
const verifyStudent = async (req, res) => {
  try {
    ...
    await pool.query('UPDATE students SET verification_status=$1, verification_note=$2, verified_by=$3, verified_at=NOW() WHERE id=$4', [status, note || null, req.admin.id, req.params.id]);
...
const bulkVerify = async (req, res) => {
  try {
    ...
    const placeholders = student_ids.map((_, i) => `$${i + 3}`).join(',');
    await pool.query(`UPDATE students SET verification_status=$1, verified_by=$2, verified_at=NOW() WHERE id IN (${placeholders})`, [status, req.admin.id, ...student_ids]);
