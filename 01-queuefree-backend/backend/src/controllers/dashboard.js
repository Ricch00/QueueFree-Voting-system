...
const getDashboardStats = async (req, res) => {
  try {
    const students = await pool.query(`SELECT COUNT(*) as total, SUM(CASE WHEN verification_status='verified' THEN 1 ELSE 0 END) as verified, SUM(CASE WHEN verification_status='pending' THEN 1 ELSE 0 END) as pending, SUM(CASE WHEN verification_status='rejected' THEN 1 ELSE 0 END) as rejected, SUM(CASE WHEN is_active=true THEN 1 ELSE 0 END) as active FROM students`);
    ...
    res.json({ success: true, data: { students: students.rows[0], elections: elections.rows[0], votes: votes.rows[0].total, recent_activity: recentActivity.rows, active_elections: activeElections.rows, turnout_chart: turnoutChart.rows } });
...
const getLiveMonitoring = async (req, res) => {
  try {
    ...
    if (!election.rows.length) return res.status(404).json({ success: false, message: 'Not found' });
    ...
    res.json({ success: true, data: { election: election.rows[0], stats: { votes_cast: votes.rows[0].count, tokens_issued: tokens.rows[0].count, tokens_used: used.rows[0].count, total_eligible: eligible.rows[0].count, turnout }, votes_by_hour: byHour } });
...
const generateReport = async (req, res) => {
  try {
    ...
    for (const pos of positions.rows) {
      ...
      results.push({ position: pos, candidates: candidates.rows.map(c => ({ ...c, percentage: total > 0 ? ((c.vote_count / total) * 100).toFixed(1) : '0.0' })), total_votes: total });
    }
    ...
    res.json({ success: true, data: { election: election.rows[0], results, stats: { ...stats.rows[0], turnout: stats.rows[0].eligible > 0 ? ((stats.rows[0].total_voters / stats.rows[0].eligible) * 100).toFixed(2) : 0 }, generated_at: new Date() } });
  } catch (err) { console.error(err); res.status(500).json({ success: false, message: 'Failed' }); }
};
