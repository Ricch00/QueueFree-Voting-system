...
const changePassword = async (req, res) => {
  try {
    ...
    await pool.query('UPDATE admins SET password = ? WHERE id = ?', [await bcrypt.hash(new_password, 12), req.admin.id]);
...
const createAdmin = async (req, res) => {
  try {
    ...
    const [ex] = await pool.query('SELECT id FROM admins WHERE email = ?', [email]);
    if (ex.length) return res.status(409).json({ success: false, message: 'Email already exists' });
    const hash = await bcrypt.hash(password, 12);
    const [result] = await pool.query('INSERT INTO admins (name, email, password, role) VALUES (?, ?, ?, ?)', [name, email, hash, role || 'admin']);
