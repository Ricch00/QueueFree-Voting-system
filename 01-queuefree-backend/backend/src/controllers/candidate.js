...
const getAllCandidates = async (req, res) => {
  try {
    ...
    if (election_id) { where += ' AND c.election_id = ?'; params.push(election_id); }
    if (status) { where += ' AND c.status = ?'; params.push(status); }
    if (position_id) { where += ' AND c.position_id = ?'; params.push(position_id); }
...
const addCandidate = async (req, res) => {
  try {
    ...
    const [ex] = await pool.query('SELECT id FROM candidates WHERE election_id=? AND position_id=? AND student_id=?', [election_id, position_id, student_id]);
    if (ex.length) return res.status(409).json({ success: false, message: 'Candidate already nominated for this position' });
    const [r] = await pool.query('INSERT INTO candidates (election_id, position_id, student_id, manifesto, nickname, sort_order) VALUES (?, ?, ?, ?, ?, ?)',
      [election_id, position_id, student_id, manifesto || null, nickname || null, sort_order || 0]);
    
    let photoUrl = null;
    if (photo) {
      photoUrl = saveBase64Image(photo, `${r.insertId}-photo-${Date.now()}.${getImageExtension(photo)}`);
      await pool.query('UPDATE candidates SET photo_url = ? WHERE id = ?', [photoUrl, r.insertId]);
    }
    
    await auditLog({ actorType: 'admin', actorId: req.admin.id, actorEmail: req.admin.email, action: 'ADD_CANDIDATE', resourceType: 'candidate', resourceId: r.insertId, req });
    res.status(201).json({ success: true, message: 'Candidate added', data: { id: r.insertId, photo_url: photoUrl } });
  } catch (err) { res.status(500).json({ success: false, message: 'Failed' }); }
};
...
const updateCandidate = async (req, res) => {
  try {
    ...
    await pool.query('UPDATE candidates SET manifesto=?, nickname=?, sort_order=?, photo_url=COALESCE(?, photo_url) WHERE id=?', 
...
const approveCandidate = async (req, res) => {
  try {
    ...
    await pool.query('UPDATE candidates SET status=?, approved_by=?, approved_at=NOW(), rejection_reason=? WHERE id=?', [status, req.admin.id, rejection_reason || null, req.params.id]);
